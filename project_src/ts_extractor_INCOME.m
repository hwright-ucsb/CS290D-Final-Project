...
for i=1:size(traffic_cases,1)
    if race(i)==1
        if income(i)==1
            white_lo(wrc,:)=traffic_cases(i,:);
            wrc=wrc+1;
        elseif income(i)==2
            white_mid(wlc,:)=traffic_cases(i,:);
            wlc=wlc+1;
       elseif income(i)==3
            white_hi(whc,:)=traffic_cases(i,:);
            whc=whc+1;
        end
    elseif race(i)==2
        if income(i)==1
            black_lo(brc,:)=traffic_cases(i,:);
            brc=brc+1;
        elseif income(i)==2
            black_mid(blc,:)=traffic_cases(i,:);
            blc=blc+1;
        elseif income(i)==3
            black_hi(bhc,:)=traffic_cases(i,:);
            bhc=bhc+1;
        end
    end
end
...
for i=1:bhc
    white_lo_samp(i,:)=white_lo(wr_inds(1,i),:);
    white_mid_samp(i,:)=white_mid(wl_inds(1,i),:);
    white_hi_samp(i,:)=white_hi(wh_inds(1,i),:);
    
    black_lo_samp(i,:)=black_lo(br_inds(1,i),:);
    black_mid_samp(i,:)=black_mid(bl_inds(1,i),:);
    black_hi_samp(i,:)=black_hi(bh_inds(1,i),:);
end
